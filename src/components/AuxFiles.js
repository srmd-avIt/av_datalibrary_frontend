...
// --- ERROR FIX: formatBoolean function definition added here ---
const formatBoolean = (value) => value === true || value === 1 ? 'Yes' : (value === false || value === 0 ? 'No' : 'N/A');

const AuxFilesTable = memo(({ auxFiles, onViewDetails, selectedAuxFileId }) => { if (auxFiles.length === 0) { return <div className="no-data-message">No data found. Try adjusting your search or filters.</div>; } return ( <div className="table-container"> <table className="user-table"> <thead>
          <tr>
            <th>AUXID</th>
            <th>newauxid</th>
            <th>AuxCode</th>
...
const AuxFileDetailsPanel = memo(({ auxFile, onClose, onViewNewMediaLogDetails }) => { if (!auxFile) return null; return ( <aside className="details-panel"> <div className="details-panel-header"><h2>AuxCode Details: {auxFile.AuxCode}</h2><button type="button" className="close-btn" onClick={onClose}><FaTimes /></button></div> <div className="details-panel-body"> 
 <DetailItem label="AUXID" value={auxFile.AUXID} />
                ...
                <DetailItem label="Has Subtitle" value={auxFile.HasSubtitle} /> </div> </aside> ); });
const NewMediaLogDetailsPanel = memo(({ event, onClose, isLoading, isError, error }) => { if (isLoading) { return ( <aside className="details-panel"><div className="details-panel-header"><h2>Loading Log Details...</h2><button type="button" className="close-btn" onClick={onClose}><FaTimes /></button></div><div className="details-panel-body"><div className="loader"></div></div></aside> ); } if (isError) { return ( <aside className="details-panel"><div className="details-panel-header"><h2>Error</h2><button type="button" className="close-btn" onClick={onClose}><FaTimes /></button></div><div className="details-panel-body"><div className="error-message">{error?.message}</div></div></aside> ); } if (!event) { return ( <aside className="details-panel"><div className="details-panel-header"><h2>Not Found</h2><button type="button" className="close-btn" onClick={onClose}><FaTimes /></button></div><div className="details-panel-body"><div className="error-message">Details for this Media Log could not be found.</div></div></aside> ); } return ( <aside className="details-panel"> <div className="details-panel-header"><h2>MediaLog Details: {event.MLUniqueID}</h2><button type="button" className="close-btn" onClick={onClose}><FaTimes /></button></div> <div className="details-panel-body">
 <DetailItem label="ML Unique ID" value={event.MLUniqueID} />
            ...
            <DetailItem label="EditingStatus" value={event.EditingStatus} />
             ...
             <DetailItem label="Topic Given By" value={event.TopicGivenBy || 'N/A'} /> </div> </aside> ); });

// API Fetching
const API_BASE_URL = process.env.REACT_APP_API_URL;

const fetchAuxFiles = async () => {
  const res = await fetch(`${API_BASE_URL}/auxfiles`, {
    cache: "no-store",
    headers: {
...
function Auxfiles() {
  ...
  const filteredAuxFiles = React.useMemo(() => {
    const query = searchQuery.toLowerCase(); let results = auxFiles;
    if (query) { results = results.filter((file) => { const code = (file.AuxCode || '').toLowerCase(); const topic = (file.AuxTopic || '').toLowerCase(); const remarks = (file.NotesRemarks || '').toLowerCase(); return code.includes(query) || topic.includes(query) || remarks.includes(query); }); }
    ...
    const checkSelect = (field, filterValue) => { if (filterValue === 'All' || !filterValue) return true; const fieldValue = (field === true || field === 1) ? 'Yes' : ((field === false || field === 0) ? 'No' : field); return fieldValue === filterValue; };
    ...
    results = results.filter(file => { return Object.keys(filters).every(key => { const fieldDef = filterFields.find(f => f.key === key); if (!fieldDef) return true; const filterValue = filters[key]; const fileValue = file[key]; switch (fieldDef.type) { case 'text': return checkString(fileValue, filterValue); case 'range': case 'number': return checkRange(fileValue, filterValue); case 'select': return checkSelect(fileValue, filterValue); default: return true; } }); });
    return results;
  ...
  const handleExport = useCallback(() => { if (filteredAuxFiles.length === 0) { alert("No data to export."); return; } const headers = Object.keys(filteredAuxFiles[0]); const csvHeader = headers.join(','); const csvRows = filteredAuxFiles.map(row => headers.map(header => { const value = row[header]; const strValue = String(value === null || value === undefined ? '' : value); if (strValue.includes(',') || strValue.includes('"')) { return `"${strValue.replace(/"/g, '""')}"`; } return strValue; }).join(',')); const csvContent = [csvHeader, ...csvRows].join('\n'); const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }); const link = document.createElement("a"); const url = URL.createObjectURL(blob); link.setAttribute("href", url); link.setAttribute("download", "auxfiles_export.csv"); document.body.appendChild(link); link.click(); document.body.removeChild(link); }, [filteredAuxFiles]);

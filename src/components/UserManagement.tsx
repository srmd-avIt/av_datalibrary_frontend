...
export function UserManagement({ onRowSelect }: UserManagementProps) {
  ...
  return (
    <div className="p-6 space-y-6" style={{ background: 'radial-gradient(circle, rgba(15,23,42,1) 0%, rgba(3,7,18,1) 100%)', color: '#e2e8f0' }}>
      ...
      <Dialog open={isAddUserDialogOpen} onOpenChange={setIsAddUserDialogOpen}><DialogContent className="sm:max-w-sm border border-white/20 shadow-xl" style={{ backgroundColor: "rgba(10, 65, 116, 0.65)", backdropFilter: "blur(14px) saturate(160%)", WebkitBackdropFilter: "blur(14px) saturate(160%)", borderRadius: "1rem" }}><DialogHeader><DialogTitle className="flex items-center gap-2 text-white"><UserPlus /> Add New User</DialogTitle></DialogHeader><AddUserForm onAddUser={handleAddUser} /></DialogContent></Dialog>

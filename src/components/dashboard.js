         ...
         const handleAssistantClick = () =>
          ...
          return ( <> <div className={`sidebar-overlay ${isOpen ? "show" : ""}`} onClick={onClose}></div> 
            ...
            <div className="sidebar-header"> 
                <h3><FaTag color="#4a90e2" /> Data library</h3>
                ...
                <ul>  <SidebarItem icon={<FaHome />} text="Home" onClick={() => navigate("/")} active={location.pathname === "/"} />
                         ...
                         <div className="sidebar-profile" style={{ marginTop: 'auto', borderTop: '1px solid var(--border-color)', padding: '1rem', cursor: 'pointer' }}> <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: 'var(--text-primary)' }}> <div style={{width: '40px', height: '40px', borderRadius: '50%', backgroundColor: '#4a4f58', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><FaUser/></div> <div> <span style={{ fontWeight: 600 }}>Admin User</span> <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)'}}>Settings / Logout</div> </div> </div> </div> </aside> </> ); });
const Header = memo(({ viewTitle, onMenuClick }) => ( <header className="header">
...
// --- SHEET COMPONENT FOR CITY/COUNTRY CARDS (UNCHANGED) ---
const DataDetailsSheet = ({ isOpen, onClose, dataType, selectedItem, year }) => {
    ...
    const renderHeaderControls = () => (
        <div className="sheet-view-controls">
            <button className={`control-btn ${viewMode === 'logs' ? 'active' : ''}`} onClick={() => setViewMode('logs')}>Log View</button>
            <button className={`control-btn ${viewMode === 'events' ? 'active' : ''}`} onClick={handleGroupByEvent}>Group by Event</button>
        </div>
    );
        ...
        <ul className="details-list">
            {logs.map(item => (
                <li key={item.MLUniqueID} className="details-list-item">
                    <div className="item-header"><strong className="item-title">{item.EventName || 'Untitled Event'}</strong><span className="item-code">{item.EventCode}</span></div>
                    <div className="item-body"><p><strong>Content Detail:</strong> {item.Detail || 'N/A'}</p>{dataType === 'City' && (<p><strong>Country:</strong> {item.fkCountry || 'N/A'}</p>)}{dataType === 'Country' && (<p><strong>City:</strong> {item.fkCity || 'N/A'}</p>)}</div>
                </li>
            ))}
        </ul>
    );
        ...
        <ul className="details-list">
            {eventData.map(event => (
                <li key={event.EventCode} className="details-list-item clickable" onClick={() => handleEventClick(event)}>
                    <div className="item-header"><strong className="item-title">{event.EventName || 'Untitled Event'}</strong><span className="item-code">{event.EventCode}</span></div>
                    <div className="item-body"><p><strong>Counts:</strong> {event.ContentCount}</p><p><strong>Content:</strong> {event.ContentDetail || 'N/A'}</p><p><strong>City:</strong> {event.fkCity || 'N/A'}</p><p><strong>FromDate: </strong>{event.FromDate || 'N/A'}</p><p><strong>ToDate: </strong>{event.ToDate || 'N/A'}</p></div>
    ...
    const renderContent = () => {
        ...
        switch (viewMode) {
            case 'events': return renderEventView();
            case 'drilldown': return renderDrilldownView();
            case 'logs': default: return renderLogView(logData);
        }
    };
    
    if (!isOpen) return null;
    const sheetTitle = `Event Logs in ${selectedItem} (${year})`;
    return (<div className="sheet-overlay" onClick={onClose}><div className={`sheet-content ${isOpen ? 'open' : ''}`} onClick={(e) => e.stopPropagation()}><div className="sheet-header"><h2>{sheetTitle}</h2><button onClick={onClose} className="close-sheet-btn"><FaTimes /></button></div>{renderHeaderControls()}<div className="sheet-body">{renderContent()}</div></div></div>);
};

// --- MODIFIED SHEET COMPONENT FOR MAP POPUPS (WITH GROUP BY FUNCTIONALITY) ---
const MapEventDetailsSheet = ({ isOpen, onClose, dataType, selectedItem, year }) => {
        ...
        if (isOpen && (dataType === 'Pratishtha' || dataType === 'Padhramani') && selectedItem && year > 0) {
            const fetchInitialLogs = async () => {
                ...
                try {
                    ...
                    if (!response.ok) throw new Error('Failed to fetch data.');
                    const data = await response.json();
                    setLogData(data); // This is our full list of logs
                } catch (err) {
                    setError(err.message);
                } finally {
                    setIsLoading(false);
    ...
    const handleGroupByEvent = async () => {
        if (eventData.length > 0) { // Avoid re-fetching if data is already there
             setViewMode('events');
             return;
        }
        setIsLoading(true);
        ...
        const endpoint = dataType === 'Pratishtha'
            ? `/dashboard/pratishtha-events-by-group?year=${year}&country=${encodeURIComponent(selectedItem)}`
            : `/dashboard/padhramani-events-by-group?year=${year}&country=${encodeURIComponent(selectedItem)}`;
    ...
    const renderLogView = (logs) => (
        <ul className="details-list">
            {logs.map((item, index) => (
                 <li key={item.MLUniqueID || `${item.EventCode}-${index}`} className="details-list-item">
                    <div className="item-header">
                        <strong className="item-title">{item.EventName || 'Untitled Event'}</strong>
                        <span className="item-code">{item.EventCode}</span>
                    </div>
                    <div className="item-body">
                      <p><strong>Content Detail:</strong> {item.Detail || item.ContentDetails || 'N/A'}</p>
            ...
            {eventData.map(event => (
                <li key={event.EventCode} className="details-list-item clickable" onClick={() => handleEventClick(event)}>
                    ...
                    <div className="item-body">
                        ...
                        <p><strong>Date:</strong> {event.FromDate ? new Date(event.FromDate).toLocaleDateString() : 'N/A'}</p>
                        <p><strong>Log Count:</strong> {event.ContentCount}</p>
                    </div>
                    <div className="item-footer">Click to see all {event.ContentCount} log entries &rarr;</div>
                </li>
            ))}
        </ul>
    );
    ...
    const renderContent = () => {
        ...
        switch (viewMode) {
            case 'events':
                return eventData.length > 0 ? renderEventView() : <div className="sheet-feedback">No grouped events found.</div>;
            case 'drilldown':
                return renderDrilldownView();
            case 'logs':
            default:
                return logData.length > 0 ? renderLogView(logData) : <div className="sheet-feedback">No logs found.</div>;
        }
    };
    
    if (!isOpen) return null;
    
    const sheetTitle = `${dataType} Logs in ${selectedItem} (${year})`;
...
function Dashboard() {
    ...
    // State for the sheet for City/Country cards
    ...
    // New state and handlers exclusively for the map popup sheet
                  ...
                  <div className="dashboard-content-wrapper">
                    ...
                    <div className="charts-grid1">
                         <DynamicLeafletMap 
                            ...
                            error={worldMapError} 
                            onPopupClick={handleMapPopupClick}
                        />
                    </div>
                  </div>
                </div>

...
const filterFields = [
  ...
  { key: 'Qcstatus', label: 'Qc Status', type: 'text' }, // Changed to 'select'
  ...
  { key: 'IsInformal', label: 'Is Informal', type: 'text' }, // Changed to 'select'
...
const EventsTable = memo(({ events, onViewDetails, selectedEventId }) => { if (events.length === 0) { return <div className="no-data-message">No data found.</div>; } return ( <div className="table-container"> <table className="user-table"> 
<thead>
                    <tr>
                        <th>fkEventCode</th>
                         <th>RecordingName</th>
 ...
 {events.map((event) => (
                        <tr key={event.RecordingCode} className={`main-row ${selectedEventId === event.RecordingCode ? 'selected' : ''}`} onClick={() => onViewDetails(event)}>
                           ...
                           <td data-label="IsInformal">{event.IsInformal ||'N/A'}</td>
...
const RecordingDetailsPanel = memo(({ event, onClose, onViewEventDetails }) => { if (!event) return null; return ( <aside className="details-panel"> <div className="details-panel-header"><h2>Recording Details: {event.RecordingCode}</h2><button type="button" className="close-btn" onClick={onClose}><FaTimes /></button></div> <div className="details-panel-body"> <div className="detail-view-group"><label>Event Code</label>{event.fkEventCode ? (<button className="link-button" onClick={() => onViewEventDetails(event.fkEventCode)}>{event.fkEventCode} <FaChevronRight size="1.2em" style={{ marginLeft: '190px', opacity: 0.7 }}/></button>) : ( <span>N/A</span> )}</div>  <DetailItem label="Recording Name" value={event.RecordingName} />
...
const EventDetailsPanel = memo(({ event, onClose, isLoading, isError, error }) => { if (isLoading) { return ( <aside className="details-panel"><div className="details-panel-header"><h2>Loading Event...</h2><button type="button" className="close-btn" onClick={onClose}><FaTimes /></button></div><div className="details-panel-body"><div className="loader"></div></div></aside> ); } if (isError) { return ( <aside className="details-panel"><div className="details-panel-header"><h2>Error</h2><button type="button" className="close-btn" onClick={onClose}><FaTimes /></button></div><div className="details-panel-body"><div className="error-message">{error.message}</div></div></aside> ); } if (!event) { return ( <aside className="details-panel"><div className="details-panel-header"><h2>Not Found</h2><button type="button" className="close-btn" onClick={onClose}><FaTimes /></button></div><div className="details-panel-body"><div className="error-message">Event details could not be found.</div></div></aside> ); } return ( <aside className="details-panel"> <div className="details-panel-header"><h2>Event Details: {event.EventCode}</h2><button type="button" className="close-btn" onClick={onClose}><FaTimes /></button></div> <div className="details-panel-body">  <DetailItem label="Event ID" value={event.EventID} />
...
const fetchDigitalRecordings = async () => {
  const res = await fetch(`${API_BASE_URL}/digitalrecording`, {
    cache: "no-store",
    headers: {
...
function Digitalrecording() {
  ...
  const handleOpenAssistant = useCallback(() => {
    ...
    setSelectedEventCodeForDetails(null);
  ...
  const filteredEvents = React.useMemo(() => {
    const query = searchQuery.toLowerCase(); let results = recordings;
    if (query) { results = results.filter((e) => ((e.fkEventCode?.toLowerCase().includes(query) || e.RecordingName?.toLowerCase().includes(query)))); }
    ...
    const checkSelect = (field, filterValue) => { if (filterValue === 'All' || !filterValue) return true; const fieldValue = field === true ? 'Yes' : (field === false ? 'No' : field); return fieldValue === filterValue; };
    const checkRange = (value, range) => { if (!range || (range.min === '' && range.max === '')) return true; const numValue = parseInt(value, 10); if (isNaN(numValue)) return false; const min = range.min !== '' ? parseInt(range.min, 10) : -Infinity; const max = range.max !== '' ? parseInt(range.max, 10) : Infinity; return (isNaN(min) || numValue >= min) && (isNaN(max) || numValue <= max); };
    results = results.filter(recording => { return Object.keys(filters).every(key => { const fieldDef = filterFields.find(f => f.key === key); if (!fieldDef) return true; const filterValue = filters[key]; const recordValue = recording[key]; switch (fieldDef.type) { case 'text': return checkString(recordValue, filterValue); case 'range': return checkRange(recordValue, filterValue); case 'select': return checkSelect(recordValue, filterValue); default: return true; } }); });
    return results;
  ...
  const handleExport = useCallback(() => { if (filteredEvents.length === 0) { alert("No data to export."); return; } const headers = Object.keys(filteredEvents[0]); const csvHeader = headers.join(','); const csvRows = filteredEvents.map(event => headers.map(header => { const value = event[header]; const strValue = String(value === null || value === undefined ? '' : value); return strValue.includes(',') ? `"${strValue}"` : strValue; }).join(',')); const csvContent = [csvHeader, ...csvRows].join('\n'); const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'Digital_Recording_export.csv'; link.click(); URL.revokeObjectURL(link.href); }, [filteredEvents]);

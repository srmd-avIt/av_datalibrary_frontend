...
// --- CORRECTED: fetchNewMediaLog now supports server-side operations ---
export async function fetchNewMediaLog({ page, limit, searchQuery, filters }) {
  ...
  const res = await fetch(`${API_BASE_URL}/newmedialog?${params.toString()}`, {
    cache: "no-store",
  ...
  try {
    return JSON.parse(text); // Expects { data: [...], totalPages: X }
  } catch (e) {
...
function Newmedialog() {
  ...
  // --- CORRECTED: useQuery is now dynamic and drives API calls ---
  const { data: paginatedData, isLoading, error } = useQuery({
    queryKey: ['newMediaLog', currentPage, searchQuery, filters],
    queryFn: () => fetchNewMediaLog({ page: currentPage, limit: eventsPerPage, searchQuery, filters }),
    keepPreviousData: true,
  });

  // --- CORRECTED: Extract data and total pages from the API response object ---
  ...
  // --- CORRECTED: useEffect resets page to 1 when filters change ---
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, filters]);
  ...
  // --- CORRECTED: ALL client-side filtering logic (React.useMemo) is removed ---

  // --- CORRECTED: Export handler is updated for server-side ---

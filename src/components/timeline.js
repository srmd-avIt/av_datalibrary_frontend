...
import AssistantPanel from './AssistantPanel'; // Import the Assistant
...
const TimelineControls = memo(({ groupBy, onGroupByChange, viewMode, onViewModeChange }) => ( <div className="view-controls"> <div className="view-controls-group"> <label htmlFor="group-by-select"><FaClock/> Group By:</label> <select id="group-by-select" className="control-select" value={groupBy} onChange={e => onGroupByChange(e.target.value)}> <option value="Yr">Year</option> <option value="EventMonth">Event Month</option> <option value="OnThisDay">On This Day (Month-Day)</option> <option value="SubmittedDate">Submission Date</option> </select> </div> <div className="view-mode-toggle"> <button className={`view-mode-btn ${viewMode === 'timeline' ? 'active' : ''}`} onClick={() => onViewModeChange('timeline')} title="Timeline View"> <FaStream /> </button> <button className={`view-mode-btn ${viewMode === 'grid' ? 'active' : ''}`} onClick={() => onViewModeChange('grid')} title="Grid View"> <FaThLarge /> </button> </div> </div> ));
const TimelineView = memo(({ events, onViewDetails, selectedEventId, groupBy }) => { const { groupedEvents, sortedGroupKeys } = useMemo(() => { if (events.length === 0) return { groupedEvents: {}, sortedGroupKeys: [] }; const getGroupKey = (event) => { let date; switch (groupBy) { case 'OnThisDay': date = new Date(event.FromDate); return isValid(date) ? format(date, 'MM-dd') : 'Invalid Date'; case 'EventMonth': date = new Date(event.FromDate); return isValid(date) ? format(date, 'yyyy-MM (MMMM)') : 'Invalid Date'; case 'SubmittedDate': date = new Date(event.SubmittedDate); return isValid(date) ? format(date, 'yyyy-MM-dd') : 'Invalid Date'; case 'Yr': default: return event.Yr || 'Unknown Year'; } }; const grouped = events.reduce((acc, event) => { const key = getGroupKey(event); if (key !== 'Invalid Date') { if (!acc[key]) acc[key] = []; acc[key].push(event); } return acc; }, {}); const sortedKeys = Object.keys(grouped).sort((a, b) => { if (groupBy === 'OnThisDay') { const todayKey = format(new Date(), 'MM-dd'); if (a === todayKey) return -1; if (b === todayKey) return 1; return a.localeCompare(b); } return b.localeCompare(a); }); return { groupedEvents: grouped, sortedGroupKeys: sortedKeys }; }, [events, groupBy]); if (sortedGroupKeys.length === 0) return <div className="no-data-message">No events found. Try adjusting your search or filters.</div>; return ( <div className="timeline-container professional"> {sortedGroupKeys.map(key => { let displayHeader = key; if (groupBy === 'OnThisDay' && isValid(parse(key, 'MM-dd', new Date()))) { displayHeader = format(parse(key, 'MM-dd', new Date()), 'MMMM dd'); } return ( <div key={key} className="timeline-section"> <h2 className="timeline-header">{displayHeader}</h2> <ul className="timeline-list"> {groupedEvents[key] .sort((a, b) => new Date(b.FromDate) - new Date(a.FromDate)) .map(event => ( <li key={event.EventID} className={`timeline-item ${selectedEventId === event.EventID ? 'selected' : ''}`} onClick={() => onViewDetails(event)}> <div className="timeline-marker"></div> <div className="timeline-content"> <h3 className="timeline-title">{event.EventName || 'Untitled Event'}</h3> <div className="timeline-meta"> <span> <FaCalendarAlt /> {event.FromDate || 'N/A'} {event.ToDate && event.ToDate !== event.FromDate ? ` - ${event.ToDate}` : ''} </span> <span> <FaHashtag /> {event.EventCode || 'N/A'} </span> </div> <div className="timeline-tags"> {event.fkEventCategory && <span className="tag-pill category">{event.fkEventCategory}</span>} {event.IsAudioRecorded === true && <span className="tag-pill audio"><FaCheckCircle /> Audio Recorded</span>} </div> </div> </li> ))} </ul> </div> ) })} </div> ); });
const GridView = memo(({ events, onViewDetails, selectedEventId, groupBy }) => { const { groupedEvents, sortedGroupKeys } = useMemo(() => { if (events.length === 0) return { groupedEvents: {}, sortedGroupKeys: [] }; const getGroupKey = (event) => { let date; switch (groupBy) { case 'OnThisDay': date = new Date(event.FromDate); return isValid(date) ? format(date, 'MM-dd') : 'Invalid Date'; case 'EventMonth': date = new Date(event.FromDate); return isValid(date) ? format(date, 'yyyy-MM (MMMM)') : 'Invalid Date'; case 'SubmittedDate': date = new Date(event.SubmittedDate); return isValid(date) ? format(date, 'yyyy-MM-dd') : 'Invalid Date'; case 'Yr': default: return event.Yr || 'Unknown Year'; } }; const grouped = events.reduce((acc, event) => { const key = getGroupKey(event); if (key !== 'Invalid Date') { if (!acc[key]) acc[key] = []; acc[key].push(event); } return acc; }, {}); const sortedKeys = Object.keys(grouped).sort((a, b) => { if (groupBy === 'OnThisDay') { const todayKey = format(new Date(), 'MM-dd'); if (a === todayKey) return -1; if (b === todayKey) return 1; return a.localeCompare(b); } return b.localeCompare(a); }); return { groupedEvents: grouped, sortedGroupKeys: sortedKeys }; }, [events, groupBy]); const formatCardDate = (dateString) => { const date = new Date(dateString); return isValid(date) ? format(date, 'MMM dd, yyyy') : 'Invalid Date'; }; if (sortedGroupKeys.length === 0) return <div className="no-data-message">No events found. Try adjusting your search or filters.</div>; return ( <div className="timeline-container"> {sortedGroupKeys.map(key => { let displayHeader = key; if (groupBy === 'OnThisDay' && isValid(parse(key, 'MM-dd', new Date()))) { displayHeader = format(parse(key, 'MM-dd', new Date()), 'MMMM dd'); } return ( <div key={key} className="timeline-section"> <h2 className="timeline-header">{displayHeader}</h2> <div className="grid-container"> {groupedEvents[key] .sort((a, b) => new Date(b.FromDate) - new Date(a.FromDate)) .map(event => ( <div key={event.EventID} className={`grid-card ${selectedEventId === event.EventID ? 'selected' : ''}`} onClick={() => onViewDetails(event)}> <div className="grid-card-year">{event.Yr || '----'}</div> <h3 className="grid-card-title">{event.EventName || 'Untitled Event'}</h3> <div className="grid-card-meta"> <span><FaCalendarAlt /> {formatCardDate(event.FromDate)}</span> <span><FaHashtag /> {event.EventCode || 'N/A'}</span> </div> <div className="grid-card-footer"> {event.fkEventCategory && <span className="tag-pill category">{event.fkEventCategory}</span>} </div> </div> ))} </div> </div> ) })} </div> ); });
...
function Timeline() {
  ...
  const handleSetViewOptions = useCallback((options) => {
    if (options.groupBy) setGroupBy(options.groupBy);
    if (options.viewMode) setViewMode(options.viewMode);
  }, []);

  const filteredEvents = useMemo(() => { const query = searchQuery.toLowerCase(); let results = events; if (query) { results = results.filter((event) => { const eventCode = event.EventCode ? event.EventCode.toLowerCase() : ''; const eventName = event.EventName ? event.EventName.toLowerCase() : ''; const year = event.Yr ? String(event.Yr) : ''; return eventCode.includes(query) || year.includes(query) || eventName.includes(query); }); } const checkString = (field, filterValue) => { if (!filterValue) return true; const fieldValue = (field || '').toString().toLowerCase(); return fieldValue.includes(filterValue.toLowerCase()); }; const checkSelect = (field, filterValue) => { if (filterValue === 'All' || !filterValue) return true; const fieldValue = field === true ? 'Yes' : (field === false ? 'No' : field); return fieldValue === filterValue; }; const checkRange = (value, range) => { if (!range || (range.min === '' && range.max === '')) return true; const numValue = parseInt(value, 10); if (isNaN(numValue)) return false; const min = range.min !== '' ? parseInt(range.min, 10) : -Infinity; const max = range.max !== '' ? parseInt(range.max, 10) : Infinity; const isMinOk = isNaN(min) || numValue >= min; const isMaxOk = isNaN(max) || numValue <= max; return isMinOk && isMaxOk; }; results = results.filter(event => { return Object.keys(filters).every(key => { const fieldDef = filterFields.find(f => f.key === key); if (!fieldDef) return true; const filterValue = filters[key]; const eventValue = event[key]; switch (fieldDef.type) { case 'text': return checkString(eventValue, filterValue); case 'range': return checkRange(eventValue, filterValue); case 'select': return checkSelect(eventValue, filterValue); default: return true; } }); }); return results; }, [events, searchQuery, filters]);
  
  const totalPages = Math.ceil(filteredEvents.length / eventsPerPage);
  const currentEvents = filteredEvents.slice((currentPage - 1) * eventsPerPage, currentPage * eventsPerPage);

  const handleExport = useCallback(() => { if (filteredEvents.length === 0) { alert("No data to export."); return; } const headers = Object.keys(filteredEvents[0]); const csvContent = "data:text/csv;charset=utf-8," + [ headers.join(','), ...filteredEvents.map(event => headers.map(header => `"${String(event[header]).replace(/"/g, '""')}"`).join(',')) ].join('\n'); const encodedUri = encodeURI(csvContent); const link = document.createElement("a"); link.setAttribute("href", encodedUri); link.setAttribute("download", "timeline_events_export.csv"); document.body.appendChild(link); link.click(); document.body.removeChild(link); }, [filteredEvents]);
  ...
  return (
    <div className={`layout ${menuOpen ? 'sidebar-open' : ''}`}>
      <Sidebar isOpen={menuOpen} setViewTitle={() => {}} onClose={handleCloseSidebar} navigate={navigate} onOpenAssistant={handleOpenAssistant} isAssistantOpen={isAssistantOpen} />
      ...
      <main className="main-content">
        ...
        <TimelineControls groupBy={groupBy} onGroupByChange={setGroupBy} viewMode={viewMode} onViewModeChange={setViewMode} />
        <div className={contentAreaClasses}>
          <div className="table-wrapper">
            ...
            {!isLoading && !error && (
              <>
                {viewMode === 'timeline' ? ( <TimelineView events={currentEvents} onViewDetails={handleSelectEvent} selectedEventId={selectedEvent?.EventID} groupBy={groupBy} /> ) : ( <GridView events={currentEvents} onViewDetails={handleSelectEvent} selectedEventId={selectedEvent?.EventID} groupBy={groupBy} /> )}
          ...
          <AssistantPanel
            ...
            onSetViewOptions={handleSetViewOptions}
          />
